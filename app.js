...
/* ============ 家長提醒 Reminders (local only, Phase 1) ============ */
function renderReminders(){
  const items = store.get(LS_KEYS.reminders, []);
  ...
  list.querySelectorAll('.rm-toggle').forEach(t => {
    t.addEventListener('click', () => {
      const items2 = store.get(LS_KEYS.reminders, []);
      const it = items2.find(x=>x.id===t.dataset.id);
      it.on = !it.on;
      store.set(LS_KEYS.reminders, items2);
      renderReminders();
    });
  });
  list.querySelectorAll('button[data-del]').forEach(b => {
    b.addEventListener('click', () => {
      const all = store.get(LS_KEYS.reminders, []).filter(x=>x.id!==b.dataset.del);
      store.set(LS_KEYS.reminders, all);
      renderReminders(); toast('已刪除提醒');
    });
  });
}
...
function initReminders(){
  document.getElementById('btn-add-reminder').addEventListener('click', () => {
    openModal('新增家長提醒', `
      <p class="muted small">此版本的提醒會在你打開這個 App、且時間已到時顯示通知。若要跨手機即時推播，之後可升級雲端版本。</p>
    ...
    document.getElementById('rm-save').addEventListener('click', () => {
      ...
      const items = store.get(LS_KEYS.reminders, []);
      items.push({ id:'r'+Date.now(), title, date, time, on:true, fired:false });
      store.set(LS_KEYS.reminders, items);
      closeModal(); renderReminders(); toast('已新增提醒');
    });
  });

  if('Notification' in window && Notification.permission === 'default'){
    // ask politely, not blocking
...
function checkDueReminders(){
  ...
  let changed = false;
  items.forEach(it => {
    if(!it.on || it.fired || !it.date) return;
    const due = new Date(`${it.date}T${it.time || '00:00'}`);
    if(due <= now){
      it.fired = true; changed = true;
      toast(`⏰ 提醒：${it.title}`);
  ...
  if(changed){ store.set(LS_KEYS.reminders, items); renderReminders(); }
}
